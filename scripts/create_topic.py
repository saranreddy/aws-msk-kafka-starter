...
def main():
    parser = argparse.ArgumentParser(description="Create a Kafka topic on AWS MSK")
    parser.add_argument(
        "--bootstrap-servers",
        required=True,
    ...
    parser.add_argument(
        "--partitions", type=int, default=2, help="Number of partitions (default: 2)"
    )
    parser.add_argument(
        "--replication-factor",
        type=int,
    ...
    try:
        ...
        if args.list_only:
            ...
            create_topic(
                admin_client, args.topic, args.partitions, args.replication_factor
            )
    except Exception as e:
